...
struct s_last_reboot_reason *s_reboot_reason_buffer;
...
static int __init ram_console_early_init(void)
{
	...
	struct mem_desc_t sram = { 0 };
	struct s_last_reboot_reason *sbufp = NULL;
	...
	sram.start = MT6737T_SEC_SRAM_ADDR;
	sram.size = MT6737T_SEC_SRAM_SIZE;
	sbufp = ioremap(sram.start, sram.size);
	s_reboot_reason_buffer = (struct s_last_reboot_reason *)
		((unsigned long)sbufp + MT6737T_SEC_SRAM_SIZE - sizeof(struct s_last_reboot_reason));
	if (sbufp)
		sbuffer_size = sram.size;
	else {
		pr_err("sram ioremap failed, [0x%lx, 0x%lx]\n", sram.start,
		       sram.size);
		return 0;
	}
#else
